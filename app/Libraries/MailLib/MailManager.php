...
class MailManager implements FactoryContract
{
    ...
    /**
     * Create an instance of the Amazon SES V2 Swift Transport driver.
     *
    ...
    protected function createSesV2Transport(array $config)
    {
        ...
        // 先加入 SSL 憑證處理設定
        $sesConfig = $this->addSesCredentials($config);

        return new SesV2Transport(
            new SesV2Client($sesConfig),
            $config['options'] ?? []
        );
    }
    ...
    public function getDefaultDriver()
    {
        ...
        return $this->app['config']['mail.driver'] ?? $this->app['config']['mail.default'];
    }
